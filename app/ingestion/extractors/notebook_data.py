...
def extract_ipynb(path: str, raw_bytes: Optional[bytes], context: ExtractorContext) -> ExtractedDocument:
    warnings: List[str] = []
    ...
    try:
        ...
        try:
            ...
            return ExtractedDocument(doc_type="ipynb", units=[ExtractedUnit(text=text)], warnings=warnings)

    cells = notebook.get("cells", []) if isinstance(notebook, dict) else list(getattr(notebook, "cells", []))
    units: List[ExtractedUnit] = []
    for idx, cell in enumerate(cells, start=1):
        if idx > context.max_objects:
            warnings.append(f"ipynb_truncated_at_max_objects={context.max_objects}")
            break
        cell_type = (cell.get("cell_type") if isinstance(cell, dict) else getattr(cell, "cell_type", "")) or ""
...
def _table_to_units(table, context: ExtractorContext) -> List[ExtractedUnit]:
    ...
    for idx, row in enumerate(rows, start=1):
        ...
        units.append(ExtractedUnit(text=normalize_text("\n".join(pairs)), metadata={"row_number": idx}))
    return units


def extract_parquet_like(path: str, raw_bytes: Optional[bytes], context: ExtractorContext, *, kind: str) -> ExtractedDocument:
    if not context.enable_parquet:
        return ExtractedDocument(doc_type=kind, units=[], warnings=["parquet_like_disabled_by_config"])
    ...
    try:
        ...
        raise MissingDependencyError("pyarrow is required for parquet/feather/arrow ingestion") from exc
...
def extract_parquet(path: str, raw_bytes: Optional[bytes], context: ExtractorContext) -> ExtractedDocument:
    return extract_parquet_like(path, raw_bytes, context, kind="parquet")


def extract_feather(path: str, raw_bytes: Optional[bytes], context: ExtractorContext) -> ExtractedDocument:
    return extract_parquet_like(path, raw_bytes, context, kind="feather")


def extract_arrow(path: str, raw_bytes: Optional[bytes], context: ExtractorContext) -> ExtractedDocument:
