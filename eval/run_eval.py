...
def run_eval(questions_file: str, top_k: int = 6, output_file: str = "eval/results.json"):
    ...
    citation_coverage = (
        (total_cited_chunks / total_returned_chunks) if total_returned_chunks > 0 else 0.0
    )
...
def main():
    ...
    parser.add_argument(
        "--questions", default="eval/questions.jsonl", help="Path to questions JSONL file"
    )
    parser.add_argument(
        "--top-k", type=int, default=settings.CONFIG.get("top_k", 6), help="Top-K to evaluate"
    )
