...
def test_ingest_endpoints_namespace_accept_and_invalid(monkeypatch):
    ...
    def fake_ingest_folder(options):
        ...
        return {'path': options.path, 'dry_run': bool(options.dry_run), 'force': bool(options.force), 'scanned': 1, 'selected': 1, 'scan_skipped': 0, 'ingested': 1, 'skipped': 0, 'failed': 0, 'files': [{'path': 'README.md', 'status': 'ok'}]}
    ...
    with TestClient(create_app()) as client:
        assert client.post('/ingest/chunks', json={'chunks': ['a'], 'namespace': 'alpha'}).status_code == 200
        assert captured['chunks'] == 'alpha'

        assert client.post('/ingest/files', json={'paths': ['README.md'], 'namespace': 'beta'}).status_code == 200
        assert captured['files'] == 'beta'

        assert client.post('/ingest/folder', json={'path': '.', 'namespace': 'gamma'}).status_code == 200
...
def test_ingest_folder_endpoint_streaming(monkeypatch):
    ...
    with TestClient(create_app()) as client:
        resp = client.post('/ingest/folder', json={'path': '.', 'stream': True, 'request_id': 'req-123'})
        text = resp.text
        assert resp.status_code == 200
...
def test_retrieval_query_returns_answer_sources_and_citation_stats(monkeypatch):
    ...
    with TestClient(create_app()) as client:
        resp = client.post('/retrieval/query', json={'query': 'payment terms', 'top_k': 3, 'rerank': True})
        payload = resp.json()
        assert resp.status_code == 200
        assert payload['ok'] is True
